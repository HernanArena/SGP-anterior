...
EscritorioCtrl.$inject = ['$scope', 'Partes', 'File', '$location', '$cookies', 'Alert'];

function EscritorioCtrl($scope, Partes, File, $location, $cookies, Alert) {
	var vm = this;
	...
	function collectionSuccess(response) {
		...
		//pe var reverse = response.sort(compare).reverse();
		for (var i = 0; i < 5; i++) {
			//pe $scope.partes.push(reverse[i]);
			$scope.partes.push(response[i]);
